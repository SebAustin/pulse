...
// CSPRNG code generator matching the production alphabet (no I/O/0/1).
// Using 12 characters for tests to make collisions astronomically unlikely
// across repeated test runs without table resets.
const testCodeAlphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
const generateTestCode = customAlphabet(testCodeAlphabet, 12);

async function freshEvent() {
  const eventId = uid("EVT");
  // Use 12-char test codes (valid 6+ for the CODE# key; length doesn't affect
  // the DynamoDB key structure) to avoid stale-code collisions across runs.
  // The condition check on the CODE# item now enforces uniqueness at write time.
...
// ============================================================================
// FIX-2: Join code uniqueness — duplicate code rejected by ConditionalCheck
// ============================================================================

describe("join code uniqueness (FIX-2 — attribute_not_exists condition)", () => {
  it("rejects a second createEvent call with the same code (TransactionCanceledException)", async () => {
    const { event } = await freshEvent();
    const sharedCode = event.code; // reuse the same code

    const hostToken2 = generateHostToken();
    const hostTokenHash2 = hashToken(hostToken2);

    // Attempt to create another event with the identical join code.
    // The ConditionalExpression attribute_not_exists(pk) on the CODE# Put should
    // cause the transaction to be cancelled with ConditionalCheckFailed.
    await expect(
      createEvent({
        eventId: uid("EVT2"),
        title: "Duplicate Code Event",
        code: sharedCode,
        hostTokenHash: hostTokenHash2,
      })
    ).rejects.toThrow(/Transaction cancelled/i);
  });

  it("succeeds when two events use different codes", async () => {
    const { event: e1 } = await freshEvent();
    const { event: e2 } = await freshEvent();
    // Different codes, both should succeed (already verified by freshEvent completing)
...
// ============================================================================
// FIX-1: Participant cookie — sign/verify at the repository boundary
// (The route-level cookie check is exercised in the route unit/integration
//  tests; here we verify the signParticipant/verifyParticipant helpers work
//  with real participant IDs generated by the system.)
// ============================================================================

import { signParticipant, verifyParticipant, participantCookieName } from "../../src/lib/auth/participant";

describe("participant cookie helpers (FIX-1 — identity binding)", () => {
  it("round-trips: sign then verify returns participantId for correct event", () => {
    const eventId = uid("EVT");
    const participantId = `u_${nanoid(12)}`;
    const cookieValue = signParticipant(eventId, participantId);
    expect(verifyParticipant(eventId, cookieValue)).toBe(participantId);
  });

  it("returns null when cookie is verified against a different event (replay blocked)", () => {
    const eventId = uid("EVT");
    const otherEventId = uid("EVTB");
    const participantId = `u_${nanoid(12)}`;
    const cookieValue = signParticipant(eventId, participantId);
    expect(verifyParticipant(otherEventId, cookieValue)).toBeNull();
  });

  it("cookieName is unique per event", () => {
    const e1 = uid("E1");
    const e2 = uid("E2");
    expect(participantCookieName(e1)).not.toBe(participantCookieName(e2));
  });

  it("vote is recorded when participantId comes from a valid cookie", async () => {
    // Simulate the cookie-to-identity flow: register a participant, then use
    // the cookie-derived participantId to cast a vote.
    const { event } = await freshEvent();
    const moment = await launchMoment({
      eventId: event.eventId,
      momentId: uid("MOM"),
      momentType: "MC",
      question: "Cookie identity test?",
      options: ["Yes", "No"],
    });

    // Simulate what /api/join does: issue a participantId, sign it
    const participantId = `u_${nanoid(12)}`;
    const cookieValue = signParticipant(event.eventId, participantId);

    // Simulate what /api/votes does: verify the cookie to get participantId
    const verifiedId = verifyParticipant(event.eventId, cookieValue);
    expect(verifiedId).toBe(participantId);

    // Use the cookie-derived ID to record a vote (same as the route would)
    const accepted = await recordVote({
      eventId: event.eventId,
      momentId: moment.momentId,
      participantId: verifiedId!,
      option: "Yes",
    });
    expect(accepted).toBe(true);

    // A second attempt with the same cookie-derived ID is rejected (dedup)
    const duplicate = await recordVote({
      eventId: event.eventId,
      momentId: moment.momentId,
      participantId: verifiedId!,
      option: "Yes",
  ...
  it("vote is blocked when an attacker supplies a tampered cookie (null participantId)", () => {
    const eventId = uid("EVT");
    const participantId = `u_${nanoid(12)}`;
    const legitimate = signParticipant(eventId, participantId);
    // Tamper: flip last char of HMAC
    const tampered = legitimate.slice(0, -1) + (legitimate.endsWith("A") ? "B" : "A");
    expect(verifyParticipant(eventId, tampered)).toBeNull();
    // null → route returns 401, no recordVote call is made
  });
});
