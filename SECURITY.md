...
| OPENAI_API_KEY | Secret | env only (`src/lib/config.ts:58`) | Never in repo; server-side only |
| AWS credentials | Secret | none stored — OIDC-vended | See "Secure by design" |
| participantId | Pseudonymous id | server-issued HMAC cookie (`pulse_pt_<eventId>`), extracted server-side | Bound to httpOnly cookie; not client-modifiable after join |
...
|---|---|---|
| **Spoofing** | Participant identity: `participantId` is extracted from the HMAC-signed httpOnly `pulse_pt_<eventId>` cookie set at join. `displayName` is now validated through `VoteSchema` (32-char max). | **Resolved** (F-02, F-03). Anonymous by design (display name only); `participantId` is server-issued and cannot be rotated by the client after join. Residual: `PULSE_SESSION_SECRET` dev fallback must be overridden in production. |
| Spoofing | Host identity via capability token. Token is 128-bit random (`generateHostToken`), compared constant-time against stored SHA-256 (`verifyToken`, `src/lib/auth/hostToken.ts`). Edge middleware redeems the URL token into an httpOnly `SameSite=Strict` cookie and 307-redirects to the tokenless URL (F-01 resolved). | **Strong.** Entropy/hashing/timing-safety all correct. Residual: token appears in server-side access log for the one initial redemption request. |
| **Tampering** | Vote/score integrity. MC votes and trivia answers use conditional `TransactWrite` for one-vote dedup (`recordVote`, `recordTriviaAnswer`); trivia scoring is **server-authoritative** (`src/lib/moment/scoring.ts`) — no client timestamp trusted. `participantId` is extracted from the server-issued HMAC cookie (F-02 resolved); `displayName` validated via `VoteSchema` (F-03 resolved). | **Strong.** Dedup is database-enforced; `participantId` cannot be rotated by the client after join; `displayName` is length-capped. |
...
| **Information disclosure** | Can an anonymous user read host-only data? `GET /api/summary` and `/ops` require a valid token (`verifyToken`). `GET /api/events/[id]` is public and returns a projected event object via `toPublicEvent()` — `hostTokenHash` is stripped before serialization (F-04 resolved). | **No hash leak.** F-04 resolved: public event responses no longer include credential material. |
| Information disclosure | Event enumeration by code. Join codes are 6-char draws from a 32-symbol CSPRNG alphabet (`customAlphabet("ABCDEFGHJKLMNPQRSTUVWXYZ23456789", 6)`); uniqueness enforced via conditional DynamoDB Put (F-05 resolved). `/api/join` rate-limited. | **Improved.** Code space is ~1 billion; no padding or filtering. Residual: no per-code lockout across Vercel instances (see F-06). |
...
| EoP | Prompt injection (LLM). `topic` (host) and audience-submitted `words` are interpolated directly into OpenAI prompts (`src/lib/ai/openai.ts:58,86`). Output is JSON-parsed / shown as text only; the LLM has **no tools and no side effects**. | **Low.** Injection can only distort the suggestion/summary text, not trigger actions — see F-09. |
...
- `OPENAI_API_KEY` read from env (`config.ts:58`); AWS creds OIDC-vended, never stored.
...
- `next.config.ts` sets `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`
  (`no-referrer` on `/host/*`), `Permissions-Policy`, a static `Content-Security-Policy`
  (`default-src 'self'`; `script-src 'self' 'unsafe-inline'` — plus `'unsafe-eval'` in dev only;
  `style-src 'self' 'unsafe-inline'`; `connect-src 'self'`; `frame-ancestors 'none'`;
  `object-src 'none'`; `base-uri 'self'`; `form-action 'self'`), and
  `Strict-Transport-Security: max-age=63072000; includeSubDomains; preload`. **F-11 resolved.**
- CSP trade-off: `script-src` uses `'unsafe-inline'` rather than a per-request nonce. A nonce
  forces every page into dynamic rendering (Next.js only stamps nonces on dynamically rendered
  pages), which would sacrifice the static performance the landing/audience surfaces rely on.
  The residual XSS surface is negligible here — there is no `dangerouslySetInnerHTML` anywhere
  and React escapes all interpolated text — while `default-src`/`connect-src`/`object-src`/
  `base-uri`/`frame-ancestors` still constrain script origins and data exfiltration.
- `Referrer-Policy: strict-origin-when-cross-origin` reduces host-token leakage via Referer;
  residual exposure is the one server-log entry at capability-URL redemption (F-01 residual).
...
| F-11 | No Content-Security-Policy and no HSTS header (`next.config.ts:11-28`) | Tampering / Info disclosure | **MEDIUM** | Without CSP, any future XSS or injected third-party script executes unconstrained (raising the impact of F-01 token theft on host pages). Without HSTS, first-request downgrade/MITM is possible. | Add a CSP (nonce-based `script-src 'self' 'nonce-…'`; `frame-ancestors 'none'`; `connect-src 'self' https://api.openai.com`) and `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload`. Per the project web-security rules, a production CSP is required. | **Resolved** (2026-06-21) — `next.config.ts` now sets a static `Content-Security-Policy` (`default-src 'self'`; `script-src 'self' 'unsafe-inline'`, `'unsafe-eval'` in dev only; `style-src 'self' 'unsafe-inline'`; `connect-src 'self'`; `frame-ancestors 'none'`; `object-src 'none'`; `base-uri 'self'`; `form-action 'self'`) and `Strict-Transport-Security: max-age=63072000; includeSubDomains; preload`. A per-request nonce was deliberately NOT used: it forces dynamic rendering on every page (Next.js only nonces dynamically rendered output), costing static performance, while the XSS surface here is negligible (no `dangerouslySetInnerHTML`; React escapes all text). |
...
> Remediation status note: F-01 through F-05 and F-11 have been addressed in code (see Status column). F-06 through F-10, F-12 remain open recommendations. F-13 is an explicit risk acceptance. The findings table reflects the current codebase state as of 2026-06-21.
...
- **No secrets in client bundle:** server-only modules isolated; `OPENAI_API_KEY` server-side only.
...
- [x] **HIGH** F-02: participant identity bound to HMAC-signed httpOnly `pulse_pt_<eventId>` cookie issued at `POST /api/join`; vote/reaction/word handlers read `participantId` from verified cookie only. Residual: `PULSE_SESSION_SECRET` dev fallback is insecure — must be overridden in production.
- [x] **MED** F-03: `displayName` added to `VoteSchema` with 32-char max; `/api/votes` reads it from `parsed.data`, not raw body.
- [x] **MED** F-04: `toPublicEvent()` strips `hostTokenHash` from all public event response DTOs.
- [x] **MED** F-05: join codes use `customAlphabet("ABCDEFGHJKLMNPQRSTUVWXYZ23456789", 6)` (CSPRNG, no filtering, no padding); uniqueness enforced by conditional DynamoDB Put with up to `MAX_CODE_RETRIES = 5` retries.
...
- [x] **MED** F-11: static CSP (`script-src 'self' 'unsafe-inline'`, `frame-ancestors 'none'`, `object-src 'none'`, `base-uri 'self'`) and `Strict-Transport-Security: max-age=63072000; includeSubDomains; preload` added to `next.config.ts`. (Nonce deliberately avoided — see F-11 row.)
